...
<div class="fr right-wrap">
    ...
    <div class="setting-panel">
        <div class="tips-area">
            ...
            为确保交易安全，保障您的合法权益，链克钱包地址一旦设置成功，不予修改。
            ...
            <div class="content normal-form form-cont clear-float">
                ...
                <div class="form-item input-cont">
                    ...
                    <input type="text" style="width:500px;" name="wkcaddr" id="wkcaddr" class="input"
                           placeholder="请输入链克钱包地址" value="<?php echo ($userinfo['wkcaddress']); ?>" <?php echo ($userinfo['wkcaddress']?'disabled':''); ?>>
                    <div class="form-tips"></div>
                    <div class="error-msg">请输入正确的链克钱包地址</div>
                    <?php if($userinfo["wkcaddress"] == ''): ?><p>使用您输入的钱包地址,向 <?php echo C("WKC_PAY_ADDRESS")[array_rand(C("WKC_PAY_ADDRESS"))];?> 转入 <?php echo mt_rand(111,999)/10000;?> 链克</p><?php endif; ?>
                </div>
                <?php echo ($userinfo['wkcaddress']?'<a class="btn  jb">解绑</a>':' <a class="btn submit">确定</a>'); ?>
...
<script>
    $('.submit').click(function () {
        ...
        $.post('wkcaddr', {wkcaddr: wkcaddr}, function (data) {
            if (data.status == 1) {
                layer.msg(data.info, {
                    icon: 1
                }, function () {
                    location.href = "<?php echo U('Setting/wkcaddr');?>";
                });
            } else {
                layer.msg(data.info, {
                    icon: 2
    ...
    $('.jb').click(function () {
        layer.open({
            ...
            yes: function (index) {
                var pwd = $('#pwd').val();
                $.get('<?php echo U("setting/wkcaddr");?>', {action: "unbind", pwd: pwd}, function (data) {
